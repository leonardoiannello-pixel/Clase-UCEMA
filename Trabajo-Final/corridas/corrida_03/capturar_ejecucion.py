...
def main():
    ...
    previous=json.loads((P/'corridas/corrida_02/reviewed_inputs.json').read_text(encoding='utf-8'))
    for entry in previous:
        assert (REVIEWED/entry['archivo']).read_bytes()==(P/'corridas/corrida_02'/entry['copia']).read_bytes()
        assert sha(REVIEWED/entry['archivo'])==entry['sha256']
    for entry in json.loads((P/'corridas/corrida_02/original_confiable.json').read_text(encoding='utf-8')):
        assert sha(ORIGINAL/entry['archivo'])==entry['sha256']
    tracked=subprocess.check_output(['git','ls-files','-z'],cwd=R).decode().split('\0')
    baseline={n:sha(R/n) for n in tracked if n and not n.startswith('Trabajo-Final/corridas/corrida_03/')}
    save(E/'fuentes_intactas_antes.json',baseline)
    shutil.copyfile(P/'prompts/system_prompt.md',E/'prompts/system_prompt.md')
    (E/'fuentes').mkdir()
    for n,p in {'cli.py':P/'agente/cli.py','contrato_herramienta.json':P/'agente/contrato_herramienta.json','v41_referencias.json':P/'agente/v41_referencias.json','v4_referencias.json':P/'agente/v4_referencias.json','inputs_referencias.json':P/'inputs/referencias.json'}.items():
    ...
    for n in NAMES:
        ...
        w.close()
    for entry in inputs:
        entry['mismos_bytes_corrida02']=True
        entry['referencia_corrida02']='../corrida_02/reviewed/'+entry['archivo']
    ...
    output=P/'ejecuciones'/('corrida_03_consolidacion_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ'))
    ...
    record.update(estado=report['estado'],source_commit=subprocess.check_output(['git','rev-parse','HEAD'],cwd=R,text=True).strip(),
        workflow_version=report.get('workflow_version'),host=host['host'],modelo=None,proveedor=None,host_metadata='host_metadata.json',datos='SINTETICOS',
    ...
    save(E/'control_integridad.json',dict(archivos_previos_verificados=len(baseline),fuentes_intactas=True,reviewed_preservados=True,metodo='SHA256 antes/después de todos los archivos versionados fuera de corrida_03 y de los cuatro reviewed.'))
    count=0
    for p in E.rglob('*'):
        if p.is_file():
